...
public class Java_quiz_2 : MonoBehaviour {
    ...
    Java_answer_checker answer_result;

    void Start()
    {
        answer_result = GameObject.Find("Question_counter").GetComponent<Java_answer_checker>();

        if (PlayerPrefs.GetString("quest_java_2") == "Completed")
        {
            answer_result.quest_active = true;
        }
        else
        {
            answer_result.quest_active = false;
        }
    }
